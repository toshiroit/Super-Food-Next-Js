...
export default function ProductDetailContent() {
  const [isActiveShow, setIsActiveShow] = useState(1)
  const dataFk = [
    {
      title: 'Mô tả',
    ...
    },
    {
      ...
      content: "Simply follow the setup prompts and the CLI will create your new project director",
      isActive: 2
    },
    {
      title: 'Trả hàng',
  ...
  const selectText = (value) => {
    ...
    let result = dataFk.map((item, index) => {
      return (
        <li key={index} onClick={() => setIsActiveShow(item.isActive)} className="table__select___item">{item.title}</li>
      )
    })
    return result;
  }
