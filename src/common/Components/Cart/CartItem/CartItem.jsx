...
export default function CartItem({ dataCartShop }) {
  ...
  const onCheckProduct = (codeShop, codeProduct, isCheck) => {
    if (checkAllProductShop.length > 0) {
      let idxProductShop = checkAllProductShop.findIndex(item => item.codeShop === codeShop);
      if (idxProductShop !== -1) {
        let idxProductCheck = checkAllProductShop[idxProductShop].productCheck.findIndex(item => item.codeProduct === codeProduct)
        if (idxProductCheck !== -1) {
          let newProductCheck = [...checkAllProductShop]
          newProductCheck[idxProductShop].productCheck[idxProductCheck].isCheck = isCheck
        ...
        setCheckAllProductShop(checkAllProductShop => [...checkAllProductShop, newProductCart])
      }
  ...
  const isCheckedCart = (codeShop, codeProduct) => {
    if (checkAllProductShop) {
      let idxProductShop = checkAllProductShop.findIndex(item => item.codeShop === codeShop);
      if (idxProductShop !== -1) {
        let idxProductCheckShop = checkAllProductShop[idxProductShop].productCheck.findIndex(item => item.codeProduct === codeProduct);
        if (idxProductCheckShop !== -1) {
          return {
            codeProduct,
            isCheck: checkAllProductShop[idxProductShop].productCheck[idxProductCheckShop].isCheck,
            isCheckAll: checkAllProductShop[idxProductShop].isCheckAll
          }
        }
        else {
  ...
  return (
    <>
      {
        dataCart && dataCart.map(itemShop => {
          return (
            <li key={itemShop.codeShop} className="main__item">
              <div className="main__item___header">
                <div className="nameShop">
                  <input onClick={() => onCheckAllProductShop(itemShop.codeShop, !isCheckedAllCart(itemShop.codeShop).isCheckAll)} type="checkbox" name={`groupShop${itemShop.codeShop}`} id="" />
                  <div className="wo">
                    ...
                    return (
                      <div className="productCart">
                        <div className="productCart__checkbox">
                          {console.log()}
                          <input
                            onClick={() => onCheckProduct(itemShop.codeShop, item.code, !isCheckedCart(itemShop.codeShop, item.code).isCheck)}
                            checked={isCheckedCart(itemShop.codeShop, item.code).isCheck ? true : false}
                            type="checkbox"
                        ...
                        <div className="productCart__name productCart__nameMobile">
                          ...
                          <div className="productCart__priceMobile">
                            <div className="pricew1">
                              <span>{priceDiscount(item.price, item.discount)}</span>
                            </div>
                        ...
                        <div className="productCart__price">
                          <div className="pricew1">
                            <span>{FormatPriceVnd(priceDiscount(item.price, item.discount))}</span>
                          </div>
                          <div className="pricew2">
                        ...
                        <div className="productCart__quality">
                          <div className="inner">
                            <i onClick={() => onQualityProduct(itemShop.codeShop, item.code, item.quality - 1)} className="fa-solid fa-minus fa-size" />
                            <input type="number" value={item.quality} name="" id="" />
                            <i onClick={() => onQualityProduct(itemShop.codeShop, item.code, item.quality + 1)} className="fa-solid fa-plus fa-size" />
                          </div>
                        </div>
                        <div className="productCart__priceResult">
                          <span>{FormatPriceVnd(priceResultQuality(item.price, item.discount, item.quality))}</span>
                        </div>
